...
int main(int argc, char* argv[])
{
   ...
   String keys =
        ...
        "{k mask|./resource/image/mask.jpg   | mask image}"         
        "{f face |./resource/haarcascade/haarcascade_frontalface_alt.xml   | frontal face haarcascade}"         
        "{m mouth |./resource/haarcascade/haarcascade_mcs_mouth.xml   | mouth haarcascade}"                        
        "{help h usage ?    |      | show help message}";      
  
    CommandLineParser parser(argc, argv, keys);
    parser.about("Ear  Detection");
    if (parser.has("help")) 
    {
        parser.printMessage();
        return 0;
    ...
    String faceCascadeName = parser.get<String>("face");
    String mouthCascadeName = parser.get<String>("mouth"); 
    String mask = parser.get<String>("mask"); 
