...
int main(int argc, char* argv[])
{
   ...
   cv::String keys =
        ...
        "{c cascade |./resource/haarcascade/haarcascade_frontalface_alt.xml   | frontal face haarcascade}"         
        "{i image |./resource/image/mask.jpg   | face mask}"                        
        "{help h usage ?    |      | show help message}";      
  
    cv::CommandLineParser parser(argc, argv, keys);
    parser.about("Face  Detection");
    if (parser.has("help")) 
    {
        parser.printMessage();
        return 0;
    ...
    String faceCascadeName = parser.get<String>("cascade");
    String faceMaskImage = parser.get<String>("image"); 
    ...
    Mat faceMask = imread(faceMaskImage);
